import { UnauthorizedError, ValidationError, PermissionError, NotFoundError, } from './error.js';
...
export async function handlerGetChirp(req, res, next) {
    try {
        ...
        if (!chirpId) {
            throw new ValidationError('No Chirp Id Provided to the Get request.');
        }
...
export async function handlerDeleteChirp(req, res, next) {
    try {
        ...
        let chirpId = req.params.chirpID;
        if (!chirpId) {
            throw new ValidationError('No Chirp Id Provided to the Delete request.');
        }
        //get the chirp
        const chirp = await getChirp(chirpId);
        if (!chirp) {
        ...
        if (chirp.userId !== userId) {
            throw new PermissionError('Cannot delete that chirp as the user is not the author.');
        }
        else {
