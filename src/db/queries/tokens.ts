...
import { refresh_tokens, RefreshToken, users } from '../schema.js';
...
export function validateRefreshToken(refreshToken: RefreshToken) {
  let result = 'valid';
  ...
  if (
    ...
    //expiration date
    Math.floor(
      refreshToken.expiresAt.getMilliseconds() -
...
export async function setRevokedAtDate(refreshToken: string) {
  return db
